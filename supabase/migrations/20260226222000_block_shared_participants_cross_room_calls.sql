...
DECLARE
  ...
  has_shared_pool_active_conflict BOOLEAN := FALSE;
  room_keywords TEXT[];
  submission_prizes TEXT[];
  is_eligible BOOLEAN := FALSE;
  has_cross_room_call_conflict BOOLEAN := FALSE;
  has_cross_submission_participant_call_conflict BOOLEAN := FALSE;
  has_shared_pool_completed BOOLEAN := FALSE;
BEGIN
    ...
    IF NEW.status = 'waiting' THEN
      SELECT EXISTS (
        ...
        WHERE qe.submission_id = NEW.submission_id
          ...
          AND qe.id <> COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::UUID)
          AND qe.status IN ('waiting', 'called', 'in_progress')
          AND rooms_share_queue_pool(NEW.room_id, qe.room_id)
      ) INTO has_shared_pool_active_conflict;

      IF has_shared_pool_active_conflict THEN
        RAISE EXCEPTION 'This group is already active in this shared challenge queue';
      END IF;
    END IF;
    ...
    IF TG_OP = 'INSERT' THEN
      SELECT EXISTS (
        ...
        WHERE qe.submission_id = NEW.submission_id
          ...
          AND (
            qe.room_id = NEW.room_id
            OR rooms_share_queue_pool(NEW.room_id, qe.room_id)
          )
      ) INTO has_shared_pool_completed;

      IF has_shared_pool_completed THEN
        RAISE EXCEPTION 'This group already completed in this challenge queue';
      END IF;
    END IF;
  END IF;

  IF NEW.status IN ('called', 'in_progress', 'completed') THEN
  ...
  IF NEW.status IN ('called', 'in_progress') THEN
    ...
    SELECT EXISTS (
      SELECT 1
      FROM submission_participants sp_new
      JOIN submission_participants sp_other
        ON sp_other.participant_id = sp_new.participant_id
      JOIN queue_entries qe
        ON qe.submission_id = sp_other.submission_id
      WHERE sp_new.submission_id = NEW.submission_id
        AND qe.id <> COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::UUID)
        AND qe.submission_id <> NEW.submission_id
        AND qe.room_id <> NEW.room_id
        AND qe.status IN ('called', 'in_progress')
    ) INTO has_cross_submission_participant_call_conflict;

    IF has_cross_submission_participant_call_conflict THEN
      RAISE EXCEPTION 'A member of this group is already called or presenting in another room';
    END IF;
  END IF;
