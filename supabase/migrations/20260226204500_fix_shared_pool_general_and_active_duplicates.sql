CREATE OR REPLACE FUNCTION rooms_share_queue_pool(room_a UUID, room_b UUID)
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1
    FROM room_challenges rc_a
    JOIN challenges c_a ON c_a.id = rc_a.challenge_id
    JOIN room_challenges rc_b ON rc_b.room_id = room_b
    JOIN challenges c_b ON c_b.id = rc_b.challenge_id
    WHERE rc_a.room_id = room_a
      AND UPPER(TRIM(c_a.keyword)) <> ''
      AND UPPER(TRIM(c_b.keyword)) = UPPER(TRIM(c_a.keyword))
  );
$$ LANGUAGE sql STABLE;
